...
class VehiclesListviewActivity : AppCompatActivity() {
    private val vehicleAPI by lazy { Globals.vehicleAPI }
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_vehicles_view)
        ...
        call.enqueue(object : Callback<List<Vehicle>> {
            override fun onResponse(call: Call<List<Vehicle>>, response: Response<List<Vehicle>>) {
                if (response.isSuccessful) {
                    ...
                    when (response.code()) {
                        400 -> {
                            Toast.makeText(this@VehiclesListviewActivity, response.errorBody()?.string(), Toast.LENGTH_LONG).show()
                        }
                        else -> {
                            Toast.makeText(this@VehiclesListviewActivity, "Error: ${response.code()}", Toast.LENGTH_LONG).show()
                        }
                    }
                }
            }
            ...
            override fun onFailure(call: Call<List<Vehicle>>, t: Throwable) {
                Toast.makeText(this@VehiclesListviewActivity, t.message, Toast.LENGTH_LONG).show()
            }
        })
    }
